 ...
 * grupos. Se requiere que el usuario tenga el rol ROLE_USER o ROLE_ADMIN para
 * acceder a estos endpoints.
 */
@RestController
...
public class GrupoController {
	 ...
	 * @return ResponseEntity que contiene el objeto GrupoConAlumnosDTO si se
	 *         encuentra el grupo; de lo contrario, un ResponseEntity con estado
	 *         NOT_FOUND.
	 */
	@GetMapping("/{id}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	 ...
	 * @return ResponseEntity que contiene una lista de objetos TurnoDTO si se
	 *         encuentran turnos; de lo contrario, un ResponseEntity con estado
	 *         NOT_FOUND.
	 */
	 ...
	 * @return ResponseEntity que contiene el objeto GrupoConAlumnosDTO que
	 *         representa el grupo creado, con estado CREATED.
	 */
	@PostMapping
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	 ...
	 *                 grupo.
	 * @return ResponseEntity que contiene el objeto GrupoConAlumnosDTO que
	 *         representa el grupo actualizado, con estado OK; o un ResponseEntity
	 *         con estado NOT_FOUND si no se encuentra el grupo.
	 */
	@PutMapping("/{id}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	 ...
	 * @return ResponseEntity con estado OK si se agrega el alumno al grupo; de lo
	 *         contrario, un ResponseEntity con estado NOT_FOUND.
	 */
	@PostMapping("/{grupoId}/alumnos/{alumnoId}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	public ResponseEntity<?> agregarAlumnoAGrupo(@PathVariable @NonNull Long grupoId,
	 ...
	 * @return ResponseEntity con estado OK si se agregan el alumno al grupo; de lo
	 *         contrario, un ResponseEntity con estado NOT_FOUND.
	 */
	@PostMapping("/{grupoId}/alumnos")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	public ResponseEntity<?> agregarAlumnosAGrupo(@PathVariable @NonNull Long grupoId,
	 ...
	 * @param grupoId  El ID del grupo.
	 * @param alumnoId El ID del alumno.
	 * @return ResponseEntity con estado OK si se elimina el alumno del grupo; de lo
	 *         contrario, un ResponseEntity con estado NOT_FOUND.
	 */
	@DeleteMapping("/{grupoId}/alumnos/{alumnoId}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	public ResponseEntity<Void> eliminarAlumnoDeGrupo(@PathVariable @NonNull Long grupoId,
			@PathVariable @NonNull Long alumnoId) {
		Optional<GrupoConAlumnosDTO> grupoOptional = grupoService.obtenerGrupoConAlumnosPorId(grupoId);
		...
		boolean alumnoEncontrado = grupo.getAlumnos().stream().anyMatch(alumno -> alumno.getId().equals(alumnoId));
	 ...
	 * @return ResponseEntity con estado OK si se agrega el turno al grupo; de lo
	 *         contrario, un ResponseEntity con estado NOT_FOUND.
	 */
	@PostMapping("/{grupoId}/turnos/{turnoId}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	public ResponseEntity<?> agregarTurnoAGrupo(@PathVariable @NonNull Long grupoId,
	 ...
	 * @param grupoId El ID del grupo.
	 * @param turnoId El ID del turno.
	 * @return ResponseEntity con estado OK si se elimina el turno del grupo; de lo
	 *         contrario, un ResponseEntity con estado NOT_FOUND.
	 */
	@DeleteMapping("/{grupoId}/turnos/{turnoId}")
	@PreAuthorize("hasRole('ROLE_MANAGER') || hasRole('ROLE_ADMIN')")
	public ResponseEntity<?> eliminarTurnoDeGrupo(@PathVariable @NonNull Long grupoId,
			@PathVariable @NonNull Long turnoId) {
		try {
