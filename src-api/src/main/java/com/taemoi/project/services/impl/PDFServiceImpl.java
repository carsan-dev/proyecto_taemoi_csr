...
import com.taemoi.project.utils.GradoUtils;
...
public class PDFServiceImpl implements PDFService {
	...
	public byte[] generarPreinscripcionFirmada(Preinscripcion p) {
		...
		String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'/><style>"
				+ "@page{size:A4;margin:9mm 11mm 10mm}body{font-family:Arial,sans-serif;color:#172126;font-size:8pt;line-height:1.18}p{margin:3px 0}ol{margin:3px 0;padding-left:18px}li{margin:0 0 1px}"
				+ ".head{border-bottom:2px solid #b7272d;padding-bottom:5px;margin-bottom:7px}.head-table{width:100%;border-collapse:collapse}.head-table td{padding:0;vertical-align:middle}.logo-cell{width:22mm}.head img{display:block;width:17mm}.title-cell{padding:0 4mm!important}h1{font-size:14pt;margin:0 0 2px;text-transform:uppercase;line-height:1.05}h2{font-size:9pt;color:#b7272d;margin:0}.ref-cell{width:34mm;text-align:right;font-weight:bold;white-space:nowrap}.section{margin:5px 0}.keep{page-break-inside:avoid}.section h3{font-size:9pt;margin:4px 0 3px;border-bottom:1px solid #aeb6b9;padding-bottom:2px}.grid{width:100%;border-collapse:collapse}.grid td{border:1px solid #ccd2d4;padding:3px;text-align:left}.label{font-size:6.5pt;color:#59666b;text-transform:uppercase}.notice{background:#f4f1e9;border-left:3px solid #b7272d;padding:5px}.signature{height:16mm;max-width:62mm;object-fit:contain;border-bottom:1px solid #555}.foot{font-size:6.5pt;color:#59666b;margin-top:5px}"
	...
	private String generarSeccion(List<AlumnoDeporte> alumnos) {
		StringBuilder html = new StringBuilder();
		// Obtener el deporte de la lista (todos los alumnos son del mismo deporte)
		Deporte deporte = alumnos.isEmpty() ? Deporte.TAEKWONDO : alumnos.get(0).getDeporte();
	...
	private String generarTablaAlumnos(List<AlumnoDeporte> alumnos) {
		...
		for (AlumnoDeporte alumnoDeporte : alumnos) {
			...
			String nombreGrado = GradoUtils.getNombreGradoParaDeporte(tipoGrado, alumnoDeporte.getDeporte());
	...
	private String getPromotionGradeForSport(Alumno alumno, AlumnoDeporte alumnoDeporte) {
		...
		return GradoUtils.getNombreGradoParaDeporte(nuevoGrado.getTipoGrado(), deporte);
	}
