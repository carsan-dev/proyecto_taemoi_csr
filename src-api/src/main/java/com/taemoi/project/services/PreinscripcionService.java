...
public class PreinscripcionService {
 ...
 private java.time.LocalTime primeraHoraGrupo(Grupo g){return turnos.findByGrupo(g).stream().filter(t->t.getHoraInicio()!=null).map(Turno::getHoraInicio).min(java.time.LocalTime::compareTo).orElse(java.time.LocalTime.MAX);}
