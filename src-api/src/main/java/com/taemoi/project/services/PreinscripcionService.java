...
public class PreinscripcionService {
 ...
 public List<Map<String,Object>> grupos(Deporte deporte){return grupos.findAll().stream().filter(g->g.getDeporte()==deporte).sorted(Comparator.comparing(Grupo::getNombre,String.CASE_INSENSITIVE_ORDER)).map(g->Map.<String,Object>of("id",g.getId(),"deporte",deporte,"nombre",g.getNombre(),"rangoEdadMin",Optional.ofNullable(g.getRangoEdadMin()).orElse(0),"rangoEdadMax",Optional.ofNullable(g.getRangoEdadMax()).orElse(99),"turnos",turnos.findByGrupo(g).stream().sorted(comparadorTurnos()).map(this::turnoPublico).toList())).toList();}
 ...
 private Comparator<Turno> comparadorTurnos(){return Comparator.comparing(Turno::getDiaSemana,String.CASE_INSENSITIVE_ORDER).thenComparing(Turno::getHoraInicio);}
