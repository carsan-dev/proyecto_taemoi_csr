...
const adminPassword =
  process.env['E2E_ADMIN_PASSWORD'] ?? 'E2e-Scroll-Only-2026!';
    ...
    .poll(
      () =>
        page
          ...
          .evaluate(
            (element, target) =>
              Math.abs(element.getBoundingClientRect().top - target),
            expected,
          ),
      { timeout: 5_000 },
    )
...
async function expectAnchorVisibleBelowFixedHeader(
  page: Page,
  selector: string,
): Promise<void> {
  await expect
    .poll(
      async () =>
        page.locator(selector).evaluate((element) => {
          ...
          const reservedHeaderHeight = headers.reduce(
            (height, header) =>
              Math.max(height, header.getBoundingClientRect().height),
            0,
          );

          return (
            targetTop >= Math.max(0, visibleHeaderBottom) - 1 &&
            targetTop <=
              Math.max(visibleHeaderBottom, reservedHeaderHeight) + 16
          );
        }),
      { timeout: 5_000 },
    )
...
test('una ruta nueva empieza arriba y Atrás restaura el fondo de la página', async ({
  page,
}) => {
  await page.goto('/');
...
test('la navegación a anclas descuenta la cabecera fija', async ({ page }) => {
  ...
  await page
    ...
    .filter({ hasText: /Dónde estamos/i })
    .click();

  await expectAnchorVisibleBelowFixedHeader(page, '#map-section');
});

test('un cambio exclusivo de query params conserva el viewport', async ({
  page,
}) => {
  await loginAsAdmin(page);
  await page.goto('/alumnosListar');
  await expect(
    page.getByRole('heading', { name: /Gestión de Alumnos/i }),
  ).toBeVisible();
  ...
  const previousTop = await page
    .locator(contextAnchor)
    .evaluate((element) => element.getBoundingClientRect().top);

  await page
    .getByRole('button', { name: 'Filtrar por alumnos aptos para examen' })
    .evaluate((button: HTMLElement) => button.click());
  await expect(page).toHaveURL(/aptoParaExamen=true/);
  await expectElementTopNear(page, contextAnchor, previousTop);
});

test('guardar un alumno restaura su sección mientras el toast sigue visible', async ({
  page,
}, testInfo) => {
  await loginAsAdmin(page);
  ...
  try {
    ...
    const previousTop = await section.evaluate(
    ...
    await page.request
      .delete(`${apiUrl}/alumnos/${student.id}`, { timeout: 5_000 })
      .catch(() => undefined);
  }
});

test('la confirmación de preinscripción sube al resultado tras el render', async ({
  page,
}) => {
  await page.route('**/api/preinscripciones/public/temporada', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('**/api/preinscripciones/public/grupos/TAEKWONDO', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify([
  ...
  await page.route(
    '**/api/preinscripciones/public/configuracion/TAEKWONDO',
    (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          ...
          version: 1,
          contenido: {
            importes: 'Cuota de prueba',
            normas: Array.from({ length: 12 }, (_, i) => `Norma ${i + 1}`),
          },
        }),
      }),
  );
  await page.route('**/api/preinscripciones', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        plazaAsignada: true,
        referencia: 'E2E-SCROLL',
        tokenDocumento: 'token-e2e',
      }),
    });
  });
  ...
  await page
    ...
    .fill('preinscripcion@e2e.taemoi.test');
  const disabilityNo = page
    .locator('label.binary-option')
  ...
  await expect(
    page.getByRole('heading', { name: 'Plaza asignada' }),
  ).toBeVisible();
  await expectScrollNear(page, 0);
  await expect(page.locator('.success')).toBeFocused();
});

test('abrir y cerrar overlays bloquea y restaura el viewport real', async ({
  page,
}) => {
...
test('el menú móvil usa un bloqueo contabilizado y devuelve el scroll', async ({
  page,
}, testInfo) => {
  test.skip(
    !testInfo.project.name.includes('mobile'),
    'Solo aplica al menú móvil',
  );
  await page.goto('/');
