...
export class PreinscripcionesAdminComponent implements OnInit, OnDestroy {
  ...
  abrirFinalizacion(){if(!this.seleccion)return;this.discapacidadHistorica=this.seleccion.tieneDiscapacidad??null;const candidatos=this.seleccion.alumnosCoincidentes||[];if(this.seleccion.dni&&candidatos.length===1&&candidatos[0].nif?.toUpperCase()===this.seleccion.dni.toUpperCase())this.elegirAlumno(candidatos[0]);else if(candidatos.length===0)this.crearNuevaFicha();else{this.accionAlumno='';this.alumnoSeleccionado=undefined;this.camposActualizar.clear();}const hoy=new Date().toISOString().slice(0,10);const tarifa=this.tarifaInicial();this.datosAlta={tipoTarifa:tarifa,cuantiaTarifa:this.cuantiaEstandar(tarifa),rolFamiliar:'NINGUNO',grupoFamiliar:'',grado:'BLANCO',fechaGrado:hoy};this.mostrandoFinalizacion=true;}
  cerrarFinalizacion(){if(this.procesandoFinalizacion)return;this.mostrandoFinalizacion=false;this.camposActualizar.clear();this.accionAlumno='';this.alumnoSeleccionado=undefined;this.discapacidadHistorica=null;}
  elegirAlumno(alumno:any){this.accionAlumno='VINCULAR_EXISTENTE';this.alumnoSeleccionado=alumno;this.camposActualizar.clear();this.camposAplicables.filter(c=>this.hayDiferencia(c)).forEach(c=>this.camposActualizar.add(c.clave));}
  crearNuevaFicha(){if(this.tieneCoincidenciaDniExacta)return;this.accionAlumno='CREAR_NUEVO';this.alumnoSeleccionado=undefined;this.camposActualizar.clear();}
  ...
  get camposAplicables(){return this.camposComparables.filter(c=>(!c.soloMenor||this.esMenorSeleccionado)&&(c.clave!=='NIF'||this.tieneValor(this.seleccion?.dni))&&(c.clave!=='TELEFONO2'||this.tieneValor(this.seleccion?.telefono2)));}
  ...
  finalizar(){if(!this.seleccion||!this.finalizacionValida||this.procesandoFinalizacion)return;this.procesandoFinalizacion=true;const datosDeporte=this.requiereDatosDeporte?{...this.datosAlta,grado:this.requiereGrado?this.datosAlta.grado:null,fechaGrado:this.requiereGrado?this.datosAlta.fechaGrado:null}:null;this.api.finalizar(this.seleccion.referencia,{accionAlumno:this.accionAlumno,alumnoId:this.accionAlumno==='VINCULAR_EXISTENTE'?this.alumnoSeleccionado.id:undefined,camposActualizar:[...this.camposActualizar],discapacidadHistorica:this.seleccion.tieneDiscapacidad==null?this.discapacidadHistorica:null,datosDeporte}).subscribe({next:()=>{this.procesandoFinalizacion=false;showSuccessToast('Preinscripción finalizada: alta, deporte, grupo y turnos asignados.');this.cerrarFinalizacion();this.cerrarDetalle();this.buscar();},error:e=>{this.procesandoFinalizacion=false;showErrorToast(e?.error?.mensaje||'No se pudo finalizar la preinscripción.');}});}
