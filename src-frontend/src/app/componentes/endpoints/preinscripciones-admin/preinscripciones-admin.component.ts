...
export class PreinscripcionesAdminComponent implements OnInit, OnDestroy {
  ...
  elegirAlumno(alumno:any){this.accionAlumno='VINCULAR_EXISTENTE';this.alumnoSeleccionado=alumno;this.prepararTarifa(alumno);this.camposActualizar.clear();this.camposAplicables.filter(c=>this.hayDiferencia(c)).forEach(c=>this.camposActualizar.add(c.clave));}
  crearNuevaFicha(){if(this.tieneCoincidenciaDniExacta)return;this.accionAlumno='CREAR_NUEVO';this.alumnoSeleccionado=undefined;this.prepararTarifa();this.camposActualizar.clear();}
